                    ...
                    <div class="inner-banner-style4">
                            ...
                            <div class="advance-style3 mb30 mx-auto animate-up-2">
                                <ul class="nav nav-tabs p-0 m-0" id="myTab" role="tablist">
                                    ...
                                    <li class="nav-item" role="presentation">
                                        <a class="nav-link active  text-color-light-grey-font" href="{{route('commercial')}}">Commercial</a>
                                    </li>
                                    <li class="nav-item text-color-light-grey-font" role="presentation">
                                ...
                                @php
                                $url = request()->url(1);
                                $searchString = 'commercial-real-estate-in-';
                        ...
                        <div class="advance-search-tab mt30 mt30-lg animate-up-3">
                            ...
                            <div class="home4-icon-style mt30 d-none d-sm-flex animate-up-4">
                                ...
                                <a href="{{route('properties.categories', ['name' => Str::slug('independent house')])}}" class="d-flex align-items-center dark-color ff-heading me-4" target="_blank"><i class="icon mr10 flaticon-home-1 text-color-orange"></i> Houses</a>
                                <a href="{{route('properties.categories', ['name' => Str::slug('apartment')])}}" class="d-flex align-items-center dark-color ff-heading me-4" target="_blank"><i class="icon mr10 flaticon-corporation text-color-orange"></i> Apartments</a>
                                <a href="{{route('properties.categories', ['name' => Str::slug('villa')])}}" class="d-flex align-items-center dark-color ff-heading me-4" target="_blank"><i class="icon mr10 flaticon-network text-color-orange"></i> Villa</a>
                                <!-- <a href="" class="d-flex align-items-center dark-color ff-heading"><i class="icon mr10 flaticon-garden text-color-orange"></i> Villa</a> -->
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="img-box-10 position-relative">
                                <div class="listing-style1 mini-style bounce-y">
                                    <div class="list-content">
                                        <h6 class="list-title"><a href="{{ route('single.listing.commerical', ['name' => Str::slug($data->property_name)])}}">{{ucfirst($data->property_name)}}</a></h6>
                                        <p class="list-text">{{ ucwords($data['project_society']) }}, {{ ucwords($data['locality']) }}, {{ ucwords($data['city']) }}</p>

                                        <a href="{{ route('single.listing.commerical', ['name' => Str::slug($data->property_name)])}}" class="btn mt15 fz15">View House<i class="fal fa-arrow-right-long"></i></a>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="row pt70 pb70">
                <div class="row wow fadeInUp" data-wow-delay="100ms">
                    ...
                    <div class="col-lg-3">
                        <div class="text-start text-lg-end mb-3">
                        @if($city == null)
                            <a class="ud-btn2" href="{{route('commerical.properties')}}">See All Properties</a>
                        @else 
                        <a class="ud-btn2" href="{{url('commerical-properties', ['city' => Str::slug($city)])}}">See All Properties</a>
                        @endif
                        </div>
                    </div>
                </div>
                @foreach($randomcityDATA as $data)
                <?php $image = explode(',', $data->images); ?>
                ...
                <div class="col-sm-6 col-lg-3">
                    <div class="listing-style5">
                        ...
                        <div class="list-content">
                            @if ($data->looking_to == 'rent')
                            <div class="list-price mb-2"><?php echo formatRent($data->rent); ?>/<span>mo</span></div>
                            @else
                            <div class="list-price mb-2"><?php echo formatCost($data->cost); ?></div>
                            @endif
                            <h6 class="list-title"><a class="text-color-black" href="{{route('single.listing.commerical',['name' =>  Str::slug($data->property_name)])}}" target="_blank">{{ucfirst($data->property_name)}}</a></h6>
                            <p class="list-text">{{ ucwords($data['project_society']) }}, {{ ucwords($data['locality']) }}, {{ ucwords($data['city']) }}</p>
    ...
    $(document).ready(function() {
        $("#citySelect").change(function() {
            ...
            if (selectedCity !== 'Select city') {
                ...
                var newUrl = "commercial-real-estate-in-" + selectedCity.toLowerCase().replace(/\s/g, '_');
                if (window.location.href.includes('/commercial-real-estate-in-/')) {
                    newUrl = newUrl.replace('commercial-real-estate-in-/', '');
                }
...
<script>
    $(document).ready(function() {
        ...
        $('#search-address').keyup(function() {
            ...
            var searchString = 'commercial-real-estate-in-';
            ...
            $.ajax({
                ...
                type: 'POST',
                url: "{{route('search.commercial.in.city')}}",
                data: {
                    search: search,
                    city: city,
                    "_token": "{{ csrf_token()}}",
                ...
                success: function(response) {
                    ...
                    if (response.length === 0) {
                        ...
                        $.each(response, function(index, property) {
                            ...
                            var propertyRoute = "{{ url('commerical-search') }}/" + encodedCity;
